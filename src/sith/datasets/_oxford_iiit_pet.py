...
_FOLDER_NAME = "oxford-iiit-pet"
_IMAGES_URL = "https://thor.robots.ox.ac.uk/~vgg/data/pets/images.tar.gz"
_METADATA_URL = "https://raw.githubusercontent.com/vturrisi/disef/refs/heads/main/fine-tune/artifacts/oxford_pets/metadata.csv"
_SPLIT_URL = "https://raw.githubusercontent.com/vturrisi/disef/refs/heads/main/fine-tune/artifacts/oxford_pets/split_coop.csv"


class OxfordIIITPetDataset(ClassificationDataset):
    """The Oxford-IIIT Pet dataset for image classification."""
    ...
    def __init__(
        ...
        *,
        ...
        """Initializes the Oxford-IIIT Pet dataset.
        ...
        with open(root / "split_coop.csv") as f:
            reader = csv.DictReader(f)
            files = [Path(row["filename"]) for row in reader if row["split"] == split]

        self._labels = [
            self._class_to_idx[self._folder_to_class[file.parent.name]]
            for file in files
        ]
        self._files = [root / "images" / file.name for file in files]
    ...
    def download(cls, root: os.PathLike[str] | str, *, force: bool = False) -> None:
        ...
        download_url(_SPLIT_URL, root=root, filename="split_coop.csv")
