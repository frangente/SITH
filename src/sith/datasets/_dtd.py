...
_FOLDER_NAME = "dtd"
_IMAGES_URL = "https://www.robots.ox.ac.uk/~vgg/data/dtd/download/dtd-r1.0.1.tar.gz"
_METADATA_URL = "https://raw.githubusercontent.com/vturrisi/disef/refs/heads/main/fine-tune/artifacts/dtd/metadata.csv"
_LABELS_URL = "https://raw.githubusercontent.com/vturrisi/disef/refs/heads/main/fine-tune/artifacts/dtd/split_coop.csv"


class DescribableTexturesDataset(ClassificationDataset):
    """The Describable Textures Dataset (DTD) for image classification."""
    ...
    def __init__(
        ...
        *,
        ...
        """Initializes the Describable Textures Dataset.
        ...
        self._labels = [
            self._class_to_idx[self._folder_to_class[file_path.parent.name]]
    ...
    def download(cls, root: os.PathLike[str] | str, *, force: bool = False) -> None:
        ...
        download_and_extract_archive(
            url=_IMAGES_URL,
            download_root=root.parent,
            extract_root=root.parent,
            remove_finished=True,
        )
        shutil.rmtree(root / "imdb")
        shutil.rmtree(root / "labels")

        download_url(_METADATA_URL, root=root, filename="metadata.csv")
        download_url(_LABELS_URL, root=root, filename="split_coop.csv")
