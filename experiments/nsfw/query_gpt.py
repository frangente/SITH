...
"""Script to query GPT to classify each singular vector as NSFW-related or not."""
...
async def query_one(
    ...
    try:
        ...
        score = int(text.removeprefix("Safety Score:").strip())
    except Exception:
...
def main(args: argparse.Namespace) -> None:
    ...
    out_file = Path(__file__).parent / "nsfw-decisions" / f"{file.stem}.txt"
    out_file.parent.mkdir(exist_ok=True)
    if out_file.exists():
        print(f"Output file {out_file} already exists, skipping...")
        return

    print(f"Querying GPT for file {file}...")
    ...
    # scores in [1, 5], None for errors
    for score in range(1, 6):
        count = counter[score]
        print(f"Score {score}: {count} ({count / len(decisions) * 100:.2f}%)")
    print(f"Errors: {counter[None]} ({counter[None] / len(decisions) * 100:.2f}%)")
